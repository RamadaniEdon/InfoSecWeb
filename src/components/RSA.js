...
const RSA = () => {
    ...
    useEffect(() => {
        getFilteredAliases(token, "rsa_", name).then((keys1) => {
    ...
    const handleGenerateKeys = () => {
        ...
        generateRSAKey(token, keySize, keyAlias, randomnessSource, name).then(() => {
            setKeys(["rsa_"+keyAlias,...keys]);
            setSelectedKey("rsa_"+keyAlias);
            getRSAPrivateKey(token, "rsa_"+keyAlias, name).then((key1) => {
                var key = getData(key1);
                setPrivateKey(key);
            }).catch((error) => {
                redToast(toast, "Error fetching private key.");
                setPrivateKey("Error fetching private key.");
            });
            getRSAPublicKey("rsa_"+keyAlias).then((key1) => {
                var key = getData(key1);
                setPublicKey(key);
            }).catch((error) => {
    ...
    const handleSelectKey = (key) => {
        ...
        getRSAPrivateKey(token, key, name).then((privateK1) => {
            let privateK = getData(privateK1);
        ...
        getRSAPublicKey(key).then((publicK1) => {
            let publicK = getData(publicK1);
            setPublicKey(publicK);
            console.log("asdfsdafsdafdsaafsdaf, "+publicK)
        }).catch((error) => {
            redToast(toast, "Error fetching public key.");
            setPublicKey("Error fetching public key.");
        });
